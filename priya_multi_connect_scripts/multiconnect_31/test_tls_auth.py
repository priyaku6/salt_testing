    ...
    def configure_tacacs_servers(self):
        ...
        try:
            for server_num in [1, 2]:
                server_name = device.custom[f'nontls_server{server_num}']
                server_ip = device.custom[f'nontls_server{server_num}_ip']
    ...
    def configure_server_groups(self):
        ...
        try:
            ...
            nontls_config = [
                ...
                f" server name {device.custom['nontls_server1']}",
                f" server name {device.custom['nontls_server2']}"
            ]
