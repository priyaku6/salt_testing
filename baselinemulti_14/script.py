...
class ConfigureTacacsServerGroup(aetest.Testcase):
    """Configure TACACS two servers in the same group (non-working first)"""

    @aetest.test
    def configure_servers_and_group(self, uut, tacacs_nonworking, tacacs_working, tacacs_key, tacacs_key_type, tacacs_trustpoint_client, tacacs_trustpoint_server, tacacs_port, tacacs_timeout, tacacs_source_intf, tacacs_group, tacacs_nonworking_ip, tacacs_working_ip):
        logger.info("Configuring TACACS non-working and working servers in the same group (non-working first)")
        config = [
