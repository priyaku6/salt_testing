...
class TacacsAccountingTest(aetest.Testcase):
    ...
    def configure_tacacs(self, uut):
        """Configure TACACS server and group with single-connection"""
        ...
        try:
            ...
            logger.info("TACACS server and group configured successfully")
    ...
    def enable_accounting(self, uut):
        ...
        try:
            uut.configure(commands)
            # Enable debug in exec mode, not config mode
    ...
    def verify_accounting(self, uut):
        ...
        try:
            debug_output = uut.execute("show logging | include accounting|TACACS|ksukulka|loopback123|test_accounting")
