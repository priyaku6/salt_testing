...
class TacacsServerConfigurationTest(aetest.Testcase):
    ...
    @aetest.test
    def configure_command_accounting(self, device, custom):
        """Enable command accounting for privilege level 15 commands"""
        try:
            accounting_config = [
                f"aaa accounting commands 15 default start-stop group {custom['tacacs_server_group']}"
            ]
            device.configure(accounting_config, timeout=60)  # Increased timeout to 60 seconds
            logger.info("Command accounting configured successfully.")
        except Exception as e:
            self.failed(f"Failed to configure command accounting: {str(e)}")

    @aetest.test
    def execute_privilege_15_command(self, device):
        """Execute a privilege level 15 command and verify accounting"""
        try:
