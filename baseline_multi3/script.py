...
class TacacsTelnetAuthTest(aetest.Testcase):
    @aetest.setup
    def setup(self, uut):
    ...
    def verify_telnet_login(self, uut):
        logger.info(banner("Verifying telnet login with TACACS authentication"))
        # Disconnect and reconnect to simulate a new telnet login
        uut.disconnect()
        time.sleep(2)
        uut.connect(via='telnet')
        assert uut.connected, "Telnet login failed"
        logger.info("Telnet login with TACACS authentication successful")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="TACACS Telnet Authentication Test")
